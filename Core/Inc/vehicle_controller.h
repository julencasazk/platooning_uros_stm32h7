...
typedef struct {
   ...
   float min_spacing_m;
   // Normal spacing correction gains
   float k_dist_normal;
   float k_vel_normal;

   // Urgent (panic) spacing correction gains (used when TTC indicates danger)
   float k_dist_urgent;
   float k_vel_urgent;

   // If estimated TTC [s] is below this threshold, use urgent gains.
   float ttc_panic_s;
   ...
   bool brake_enable;
   // If > 0, brake mapping logic is only allowed when distance_to_front_m <= this.
   // Outside this range the controller falls back to the signed PID (throttle/brake).
   float brake_map_max_dist_m;
