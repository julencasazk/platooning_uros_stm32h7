                                 ...
                                 const vehicle_controller_params_t *params)
{
    ...
    if (params->brake_enable &&
        (params->brake_map_max_dist_m < 0.0f ||
    ...
    if (piderr != _PID_OK)
        return VC_ERR_PIDLIB;
    piderr = pid_set_anti_windup(pidptr, _PID_ANTI_WINDUP_INTEGRATOR_CLAMPING);
    if (piderr != _PID_OK)
        return VC_ERR_PIDLIB;
                                 ...
                                 vehicle_controller_outputs_t *out)
{
    ...
    if ((params->platoon_index > 0) && (in->distance_to_front_m > 0.0f))
    {
        ...
        // TTC calculation (positive, seconds). Large value when not closing.
        if ((in->distance_to_front_m > 0.0f) && (speed_diff_raw < -0.1f))
        {
            ttc = in->distance_to_front_m / (-1.0f * speed_diff_raw);
        }
        else
        {
        ...
        if (fabsf(speed_diff) < fmaxf(0.0f, params->vel_diff_deadband_mps))
        {
            speed_diff = 0.0f;
        }

        float d_sp = (k_dist * dist_error) + (k_vel * speed_diff);
        if (dist_error < 0.0f)
            d_sp = 4.0f * d_sp;
    ...
    // TTC calculation (when no valid distance or we're leader)
    if (!((params->platoon_index > 0) && (in->distance_to_front_m > 0.0f)))
    {
        if ((in->distance_to_front_m > 0.0f) && (speed_diff_raw < -0.1f))
        {
    ...
    const float u = (float)pid_run(&(ctrl->speed_pid), base_sp, in->speed_mps);
    throttle = u;
    ...
    // Brake mapping is only allowed within a certain distance to the predecessor.
    // However, if a brake "pulse" is already active (ctrl->brake_active), we keep
    // using the mapping until the mapping logic itself decides the pulse is over.
    const bool allow_brake_mapping =
        params->brake_enable &&
        (params->platoon_index > 0) &&
        ((ctrl->brake_active) ||
         ((in->distance_to_front_m > 0.0f) &&
          ((params->brake_map_max_dist_m <= 0.0f) || (in->distance_to_front_m <= params->brake_map_max_dist_m))));

    if (allow_brake_mapping)
    {
        // Gate checks
    ...
    // Write outputs
    // Outside the mapping range, fall back to signed PID throttle/brake.
    if (!allow_brake_mapping)
    {
        const float th_u = fmaxf(0.0f, fminf(1.0f, u));
        const float br_u = fmaxf(0.0f, fminf(1.0f, -u));
        out->throttle_cmd = th_u;
        out->brake_cmd = br_u;
        out->desired_decel_mps2 = br_u * fmaxf(0.0f, params->decel_full);
